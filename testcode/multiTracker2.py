...
# gst_src = 'filesrc location=highway2.mp4 ! qtdemux ! queue ! h264parse ! nvv4l2decoder ! nvvidconv ! video/x-raw,format=BGRx,width=640,height=360 ! queue ! videoconvert ! queue ! video/x-raw, format=BGR ! appsink'

# cap = cv2.VideoCapture(gst_src, cv2.CAP_GSTREAMER)

videoSRC = "highway2.mp4"
...
while cap.isOpened():
    ...
    if key == ord(' ') or (videoSRC!= 0 and isFirst): #(gst_src!= 0 and isFirst): 
        isFirst = False
    ...
    elif key in range(48, 56): # 0~6 숫자 입력   ---⑥
